...
def solve_maze_Dijkstra(grid_cells: List[Cell], sc: pygame.Surface):
    """
    Solve the maze using Dijkstra's Algorithm.

    Dijkstra's explores nodes in order of increasing cost from the start.
    It guarantees the shortest path but does not use any heuristic,
    so it may explore more nodes than A*.
    ...
    while open_set:
        current_cost, _, current_cell = heapq.heappop(open_set)
        ...
        draw_text_of_running_alg(sc, "RUNNING: Dijkstra", FONT, 17, 20, 200, "#FFFFFF")
